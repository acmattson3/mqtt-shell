...
- `MQTT_HOST` / `MQTT_PORT`: Overrides broker host/port (defaults to `sssn.us:1883`).
- `MQTT_PASSWORD`: MQTT broker password. If omitted, you will be prompted.
- `MQTT_AGENT_PASSWORD`: Agent password. If omitted, you will be prompted per connection.
- `MQTT_USER`: Optional MQTT username override (defaults to the username parsed from `[user@]session_id`).
...
# Optionally pre-set if you don't want to be prompted:
# export MQTT_AGENT_PASSWORD=unique-agent-secret

python3 mqtt_shell_client.py mqttuser@agent-01
```

The client will:
1) Connect to the MQTT broker with `MQTT_USER` (from the `[user@]` prefix) and `MQTT_PASSWORD`.
2) Send the `MQTT_AGENT_PASSWORD` to the agent over a dedicated auth topic.
